...
bool Directory::Add(char *name, int newSector, bool _isDir)
{
    if (FindIndex(name) != -1)
        return FALSE;
...
// Recursive List
void Directory::rList(int depth)
{
    ...
    for (int i = 0; i < tableSize; i++)
        if (table[i].inUse) {
            ...
            if (table[i].isDir) {
                ...
                Directory* subDirectory = new Directory(10);
                subDirectory->FetchFrom(subDirectoryFile);
                subDirectory->rList(depth+1);
