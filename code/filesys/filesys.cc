...
bool FileSystem::Create(char *name, int initialSize)
{
    ...
    std::vector<int> dirSectors;
    std::vector<OpenFile*> directoryFiles;
    std::vector<Directory*> directories;
    
    dirSectors.push_back(1);
    directoryFiles.push_back(directoryFile);
    directories.push_back(directory);
    

    for (int i=0; i<names.size(); i++) {
        cname = strdup(names[i].c_str());
        if (i >= names.size()-1) break;
        int dirSector1 = directories[i]->Find(cname);
        OpenFile* directoryFile1 = new OpenFile(dirSector1);
        Directory* directory1 = new Directory(NumDirEntries);
        directory1->FetchFrom(directoryFile1);

        dirSectors.push_back(dirSector1);
        directoryFiles.push_back(directoryFile1);
        directories.push_back(directory1);

    }
    freeMap = new PersistentBitmap(freeMapFile, NumSectors);
    sector = freeMap->FindAndSet();  // find a sector to hold the file header
    directories[names.size()-1]->Add(cname, sector, false);

    hdr = new FileHeader;
    hdr->Allocate(freeMap, DirectoryFileSize);
    hdr->WriteBack(sector);

    directories[names.size()-1]->WriteBack(directoryFiles[names.size()-1]);
    freeMap->WriteBack(freeMapFile);
...
bool FileSystem::CreateDir(char *name)
{
    ...
    freeMap = new PersistentBitmap(freeMapFile, NumSectors);
    sector = freeMap->FindAndSet();  // find a sector to hold the file header
    directories[names.size()-1]->Add(cname, sector, true);

    hdr = new FileHeader;
    hdr->Allocate(freeMap, DirectoryFileSize);
    hdr->WriteBack(sector);

    Directory *directoryNew = new Directory(NumDirEntries);
    OpenFile* directoryFileNew = new OpenFile(sector);        
    directoryNew->WriteBack(directoryFileNew);
...
OpenFile * FileSystem::Open(char *name)
{
    ...
    sector = directories[names.size()-1]->Find(cname);
